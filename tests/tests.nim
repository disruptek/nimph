...
block:
  ...
  testes:
    ...
    var deps {.used.} = newDependencyGroup(project, {Dry})
    ...
    var cute = proj.get
    var repository = openRepository project.gitDir
    defer:
      free repository.get
...
block:
  ...
  testes:
    ...
    check project.findProject(".")
    project.cfg = loadAllCfgs(project.repo)
    var deps {.used.} = project.newDependencyGroup({Dry})
    check project.resolve(deps)

    var path = deps.pathForName("cutelog")
    check path.isSome
    var cute = deps.projectForPath(path.get).get
